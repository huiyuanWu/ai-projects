...
def manhattan_distance(matrix, g):
    h = 0
    for i in range(0, 3):
        for j in range(0, 3):
            if matrix[i][j] != '.':
...
frontier = []
def astar(matrix):
    initial_cost = manhattan_distance(matrix, 0)
...
def getNexts(matNode, root):
    ...
    if canMoveL(x, y, mat):
        ...
        if not invArrL > invArrO:
            g = get_path_cost(matNode, root)
            cost = manhattan_distance(matL, g)
    ...
    if canMoveR(x, y, mat):
        ...
        if not invArrR > invArrO:
            g = get_path_cost(matNode, root)
            cost = manhattan_distance(matR, g)
    ...
    if canMoveU(x, y, mat):
        ...
        if not invArrU > invArrO:
            g = get_path_cost(matNode, root)
            cost = manhattan_distance(matU, g)
    ...
    if canMoveD(x, y, mat):
        ...
        if not invArrD > invArrO:
            g = get_path_cost(matNode, root)
            cost = manhattan_distance(matD, g)
