...
def uniform(matrix):
    ...
    while len(frontier) != 0:
        ...
        next_states = getNexts(node,root)
...
def getNexts(matNode, root):
    ...
    if canMoveL(x, y, mat):
        ...
        if not invArrL > invArrO:
            cost = get_path_cost(matNode, root)
...
if __name__ == '__main__':
    ...
    #print_m(matrix)
    array = matrix_to_list(matrix)
    #print(matrix_to_list(parse(path)))
    #print(check_inversion(array))
    #print(check_solvable(array))
    if check_solvable(array):
        if search == "ucost":
            print("solving with uniform cost")
            uniform(matrix)
        print("path cost:", path_cost)
        print("total nodes add to frontier: ", frontier_sum)
        print("total nodes expanded ", expanded_sum)
    else:
        print("No solution")
